...
class Actor(nn.Module):            # NOTE: 현재 Policy는 state와 latent의 concat을 input으로 받도록 할 것이다.
    ...
    def forward(self, observations: jnp.ndarray, latent: jnp.ndarray, deterministic):
    ...
    def deterministic_action(self, x: jnp.ndarray, deterministic: bool):
...
class SACPolicy(object):
    def __init__(
        ...
        actor = Model.create(
            actor_def,
            inputs=[actor_rngs, init_observation, init_latent, False],
