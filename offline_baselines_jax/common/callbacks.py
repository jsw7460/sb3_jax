...
class EvalCallback(EventCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            episode_rewards, episode_lengths, task_rewards, task_lengths = evaluate_policy(
                self.model,
                self.eval_env,
            ...
            self.logger.record("eval/mean_success_rate", mean_ep_length)

            for task_name in task_rewards.keys():
                self.logger.record("eval/{}_reward".format(task_name), float(np.mean(task_rewards[task_name])))
                self.logger.record("eval/{}_success_rate".format(task_name), np.mean(task_lengths[task_name]))
